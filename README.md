...
🔍 **원인 분석**:
Imgur와 같은 다수의 이미지 호스팅 서비스는 다른 웹사이트가 자신의 이미지 리소스를 직접 링크하여 트래픽을 유발하는 핫링킹(Hotlinking)을 방지합니다. 브라우저가 이미지를 요청할 때 보내는 Referer 헤더를 확인하여, 허가되지 않은 사이트からの 요청을 차단하는 것입니다.
