...
<body>
  ...
  <script>
    ...
    function addLegend(){
      ...
      div.innerHTML=`<div class="key"><span class="sw"></span>클릭 가능 (랜드마크 있음)</div>
                     <div class="key"><span class="sw off"></span>클릭 불가 (없음)</div>`;
  ...
  <script>
    ...
    function renderCards(root, countryName, items) {
      ...
      items.slice(0, 3).forEach(item => {
        ...
        const linkId   = encodeURIComponent(item.landmark_id ?? '');
        a.href = `${CTX}/landmarkInfo.jsp?name=${linkName}&id=${linkId}`;

        // ✅ 변환된 URL
        const imgSrc = toDirectImageUrl(imageLookup.get(item.landmark_id)) || PLACEHOLDER;
  ...
  <script>
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
        { attribution:'&copy; OpenStreetMap', noWrap:true }).addTo(map);
