    ...
    <script>
        ...
        async function buildMainImageLookup() {
            try {
                ...
                if (!res.ok) {
                    console.error('이미지 목록 로드 실패. 서버 응답 코드:', res.status);
                    return new Map();
                }
                const rows = await res.json();
                
                if (!Array.isArray(rows) || rows.length === 0) {
                    console.warn('[image] getImage API가 유효한 배열을 반환하지 않았습니다. 데이터:', rows);
                    return new Map();
                }
                ...
                for (const img of rows) {
                    if (!img || typeof img !== 'object') {
                        console.warn('유효하지 않은 이미지 데이터 항목이 발견되어 건너뜁니다:', img);
                        continue;
                    }
                    ...
                    if (type === 'main' && lid != null && url && !lookup.has(lid)) {
                        lookup.set(lid, convertImgurUrl(url));
                    }
                }
                return lookup;
            } catch (e) {
                console.error('[image] 이미지 맵 생성 중 치명적 오류 발생:', e);
                return new Map();
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                console.error("⛔️ 데이터 로딩 중 치명적인 오류 발생:", err);
                listRoot.innerHTML = `
                    <div class="card">
